...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_cpu(
  ...
  top[0]->mutable_cpu_data()[0] = (loss_pos * 1.0 + loss_neg * 1.1) / num;
}
...
void SigmoidCrossEntropyLossLayer<Dtype>::Backward_cpu(
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < num; ++i) {
    	...
    	for (int j = 0; j < dim; j ++) {
        if (target[i*dim+j] == 1) {
          bottom_diff[i * dim + j] *= 1.0 * count_neg / (count_pos + count_neg);
        }
        else if (target[i*dim+j] == 0) {
          bottom_diff[i * dim + j] *= 1.1 * count_pos / (count_pos + count_neg);
        }
        else {
