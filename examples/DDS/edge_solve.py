...
def interp_surgery(net, layers):
    for l in layers:
        ...
        if m != k:
            raise Exception('input + output channels need to be the same ' + l)
        if h != w:
...
base_weights = 'pretrained_models/edge_cls_coco_reweighted_loss_pretrained_on_imagenet_iter_80000.caffemodel'

# init
caffe.set_mode_gpu()
caffe.set_device(0)

solver = caffe.SGDSolver('edge_solver.prototxt')
...
# solver.restore('snapshots/edge_sbd_iter_10000.solverstate')
